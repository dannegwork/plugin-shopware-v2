<?php
namespace Boxalino\Components\Event;
...
class EventReporter
{
    protected $configPriceFilter = array(
        1 => array("from" => 0, "to" => 5),
    ...
    private function buildScript($pushes = null)
    {
        $account = P13NHelper::getAccount();
        return <<<SCRIPT
...
SCRIPT;
    ...
    public function reportLogin($userId)
    {
        $script = <<<SCRIPT
...
SCRIPT;
     ...
     */
    public function reportPurchase($products, $orderId, $totalPrice, $currency)
    {
...
//            }
    ...
    private function returnProperPriceRange($priceOption)
    {
        $range = $this->configPriceFilter[$priceOption];
        if($priceOption <= 1) {
    ...
    private function returnProperCategoryTree($categoryOption)
    {
        $categories = '';
    ...
    private function returnSupplierValue($supplierId)
    {
    ...
    private function returnfilterValue($paramName, $paramValue)
    {
    }
}